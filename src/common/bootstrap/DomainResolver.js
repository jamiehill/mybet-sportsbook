...
export default class DomainResolver extends DeferredBase {
	...
	initialize() {
		...
		this.urls = SportsBook.Urls = {};
		this.config = SportsBook.Config;

		this.map = {};

		// get main application domain and path
		var url  = window.location.host.replace(regEx.leadingTrailingSlashes, '');
		this.urls.path = window.location.pathname.replace(regEx.leadingTrailingSlashes, '');
		...
		_.each(this.map, function(val, key) {
			console.log('Bootstrap: '+key+' - url: '+val);
		}, this);
